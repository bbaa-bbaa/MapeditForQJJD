<template>
  <div id="Main">
    <div
      id="SelectBlock"
      v-if="!Main.MapEditer.IsPlayerEdit && IsPC"
    ...
    >
        ...
        <v-tooltip bottom v-if="blocks[Main.MapEditer.Select]">
          <template v-slot:activator="{ on }">
            <div :style="Main.MapEditer.SelectStyle">
              <img v-on="on" src="../../public/images/select.png" draggable="false" />
            </div>
          </template>
      ...
      <div id="BlockList">
        <v-row no-gutters v-for="(item, y) of Main.MapEditer.imagedata" :key="'y' + y" justify="center">
          <v-col
    ...
    <div id="PhoneSelectBlock" v-if="!IsPC">
      <v-dialog v-model="EnableSelectBlock" fullscreen hide-overlay transition="dialog-bottom-transition">
        <v-card>
          <v-card-title>
            选择方块
          </v-card-title>
          <v-card-text>
            ...
            <div id="PhoneBlockList">
              <v-row no-gutters justify="start">
                <template v-for="(item, y) of Main.MapEditer.imagedata">
                  <template v-for="(item2, x) of item">
                    <v-col
                      style="height:66px;max-width:66px;margin:0px 0px -1px -1px;border:1px solid #000;"
                      v-if="blocks[y * 15 + x]"
                      :key="y * 15 + x"
    ...
    <v-container>
        ...
        <v-form :lazy-validation="true" ref="PlayerInfo">
          ...
          <v-row>
            <v-col cols="6">
              <v-btn
                :color="
                  this.Main.PlayerEditer.data[Main.PlayerEditer.Y][Main.PlayerEditer.X].Info[2] ? 'primary' : 'success'
                "
                dense
                :block="true"
                @click="AddPlayer"
              >
                {{ this.Main.PlayerEditer.data[Main.PlayerEditer.Y][Main.PlayerEditer.X].Info[2] ? "修改" : "确定" }}
              </v-btn>
            </v-col>
            <v-col cols="6">
              <v-btn color="error" dense @click="DeletePlayer" :block="true">
                {{ this.Main.PlayerEditer.data[Main.PlayerEditer.Y][Main.PlayerEditer.X].Info[2] ? "删除" : "取消" }}
              </v-btn>
            </v-col>
          </v-row>
        </v-form>
      </div>
      <v-dialog v-model="EntityEditerOpen">
        <v-card>
          <v-card-title>
            修改实体信息
          </v-card-title>
            ...
            <v-form :lazy-validation="true" ref="PlayerInfo">
              ...
              <v-row>
                <v-col cols="6">
                  <v-btn
                    :color="
                      this.Main.PlayerEditer.data[Main.PlayerEditer.Y][Main.PlayerEditer.X].Info[2]
                        ? 'primary'
                        : 'success'
                    "
                    :block="true"
                ...
                <v-col cols="6">
                  <v-btn color="error" @click="DeletePlayer" :block="true">
                    {{
                      this.Main.PlayerEditer.data[Main.PlayerEditer.Y][Main.PlayerEditer.X].Info[2] ? "删除" : "取消"
                    }}
                  </v-btn>
      ...
      <div id="footer">
        <v-row>
          <v-col cols="12">
            <v-textarea v-model="Main.SaveFile" :auto-grow="true" :filled="true"></v-textarea>
          </v-col>
        </v-row>
        <v-row>
          <v-col>
            <v-row justify="center">
              <v-btn-toggle :max="0" :multiple="true" @click="AutoSaveF">
                <v-btn color="primary" @click="Main.MapEditer.ReSizeMap('left')">
                  在左边添加一列
                </v-btn>
                <v-btn color="primary" @click="Main.MapEditer.ReSizeMap('right')">
                  在右边添加一列
                </v-btn>
                <v-btn color="primary" @click="Main.MapEditer.ReSizeMap('top')">
                  在顶部添加一行
                </v-btn>
                <v-btn color="primary" @click="Main.MapEditer.ReSizeMap('bottom')">
                  在底部添加一行
                </v-btn>
              </v-btn-toggle>
            </v-row>
        ...
        <v-row>
          ...
          <v-col cols="6">
            <v-btn color="primary" :block="true" @click="toggleMP">
              {{ Main.MapEditer.IsPlayerEdit ? "切换到地图编辑器" : "切换到玩家编辑器" }}
            </v-btn>
          </v-col>
        </v-row>
...
<script>
  ...
  export default {
    ...
    computed: {
      HeightToTop() {
        return (this.ShowSelectBlock ? this.Main.MapEditer.imagedata.length * 66 : 0) + "px";
      },
      ShowSelectBlock() {
        return this.EnableSelectBlock && !this.Main.MapEditer.IsPlayerEdit && this.IsPC;
      },
      EntityEditerOpen: {
        get() {
          return this.Main.PlayerEditer.ShowEditer && !this.IsPC;
        },
        set(value) {
          this.Main.PlayerEditer.ShowEditer = value;
        }
      },
      hasAutoSave() {
        return "AutoSave" in localStorage;
      },
      IsPC() {
    ...
    mounted() {
      ...
      this.EnableSelectBlock = this.IsPC;
    },
    methods: {
      AutoSaveF() {
        var Save = {};
        ...
        localStorage.AutoSave = JSON.stringify(Save);
      },
      tryDetectComp(event) {
        if (event.substring(0, 2) == "wf") {
      ...
      initSelect() {
        if (this.IsPC) {
          this.Main.MapEditer.SelectStyle.left =
            document.querySelector("#BlockList > div:nth-child(1) > div:nth-child(1) > img").getBoundingClientRect()
              .left + "px";
        }
      },
      initPlayerEditer() {
        this.Main.PlayerEditer.Style.left = document.querySelector("#MapEditer").scrollLeft + "px";
        this.Main.PlayerEditer.Style.top = 0 + "px";
      },
      toggleMP() {
        Vue.set(
          this.Main.MapEditer,
          "IsPlayerEdit",
          this.Main.MapEditer.IsPlayerEdit ? this.Main.MapEditer.ToCsv() : this.Main.PlayerEditer.ToSave()
        );
      },
      PlayerEdit(x, y, el) {
        if (
        ...
        if (!el.ctrlKey) {
          ...
          if (typeof this.Main.PlayerEditer.data[y][x].Info[2] != "undefined") {
            this.Main.PlayerEditer.PlayerInfo = this.Main.PlayerEditer.data[y][x].Info;
          }
        } else {
          if (typeof this.Main.PlayerEditer.data[y][x].Info[2] != "undefined") {
            this.Main.PlayerEditer.PlayerInfo = this.Main.PlayerEditer.data[y][x].Info;
          ...
          this.Main.PlayerEditer.PlayerInfo = new Array();
          if (
            typeof this.Main.PlayerEditer.data[this.Main.PlayerEditer.Y][this.Main.PlayerEditer.X].Info[2] !=
            "undefined"
          ) {
            for (let [i, item] of this.Main.PlayerEditer.data[this.Main.PlayerEditer.Y][
        ...
        this.$refs.PlayerInfo.resetValidation();
      ...
      LoadSavea() {
        var Save = JSON.parse(localStorage.AutoSave);

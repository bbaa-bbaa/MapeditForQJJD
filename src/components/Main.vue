<template>
  <div id="Main">
        ...
        <v-tooltip bottom v-if="blocks[Main.MapEditer.Select]">
          <template v-slot:activator="{ on }">
            <div :style="Main.MapEditer.SelectStyle">
              <img v-on="on" src="../../public/images/select.png" draggable="false" data-a="A" />
            </div>
          </template>
          <div style="width:100%">{{ blocks[Main.MapEditer.Select].name }}</div>
          <div style="width:100%">{{ blocks[Main.MapEditer.Select].comment }}</div>
        </v-tooltip>
    ...
    <div id="MapEditer" style="width:100%" :style="{ 'padding-top': HeightToTop }">
      <v-row no-gutters v-for="(item, y) of Main.MapEditer.data" :key="'y' + y" justify="center">
        <v-col
          ...
          :key="x"
        >
          <img draggable="false" :src="item2.url()" onerror="this.src='./images/error.png'" @load="initPlayerEditer" />
        </v-col>
      </v-row>
    </div>
    <div
      ...
      v-if="Main.MapEditer.IsPlayerEdit"
      style="width:100%"
      :style="Object.assign(Main.PlayerEditer.Style, { 'padding-top': HeightToTop })"
    >
    ...
    <v-container>
      <div id="PlayerInfoEdit" :style="Main.PlayerEditer.EditerStyle" v-if="Main.PlayerEditer.ShowEditer">
        <v-text-field
          label="名称"
          dense
      ...
      <div id="footer">
        ...
        <v-row>
          <v-col>
            <v-btn color="primary" :block="true" @click="LoadSavea" :disabled="!getAutoSave()">
              加载自动保存
            </v-btn>
          </v-col>
        </v-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      initPlayerEditer() {
        this.Main.PlayerEditer.Style.left = document.querySelector("#MapEditer").offsetLeft + "px";
        this.Main.PlayerEditer.Style.top = document.querySelector("#MapEditer").offsetTop + "px";
      },
      toggleMP() {
        Vue.set(
          this.Main.MapEditer,
